...
class TextBox():
 def __init__(self, text, x=10, y=10, color="black", fsize=14, anchor="start", svg=None, vspace=10):
  self.text = text
  self.x=x
  self.y=y
  self.color=color
 ...
 def result(self):
  st=f'''
  <text text-anchor="{self.anchor}" fill="{self.color}"
  x="{self.x}" y="{self.y}" 
  font-size="{self.fsize}">\n'''
  d=self.text.split("\n")
  #st+=f"{d[0]}"
  for i in range(len(d)):
   st+=f'''<tspan x="{self.x}" dy="{self.fsize-self.vspace}">{d[i]}</tspan>'''
  st+="\n</text>"  
  return st